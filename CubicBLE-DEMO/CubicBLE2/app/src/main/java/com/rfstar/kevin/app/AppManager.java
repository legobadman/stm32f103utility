...
public class AppManager {
	...
	public AppManager(Context context) {
		...
		if (!context.getPackageManager().hasSystemFeature( // 检察系统是否包含蓝牙低功耗的jar包
			...
			Toast.makeText(context, R.string.do_not_contain_bluetooth_package, Toast.LENGTH_SHORT)
					.show();
			return;
		}
		...
		if (bleAdapter == null) { // 检察手机硬件是否支持蓝牙低功耗
			Toast.makeText(context, R.string.do_not_suppot_bluetooth_package, Toast.LENGTH_SHORT).show();
			return;
		}
	}
