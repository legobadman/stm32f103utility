...
public class ModuleParameterActivity extends BaseActivity implements
	...
	public void onClick(View v) {
		...
		if (v == ff92Layout) {
			this.initFF92Array();
			title = getString(R.string.connection_interval);
			type = FF92;
		} else if (v == ff93Layout) {
			this.initFF93Array();
			title = getString(R.string.serial_port);
			type = FF93;
		} else if (v == ff94Layout) {
			this.initFF94Array();
			title = getString(R.string.restore_control_channel);
			type = FF94;
		} else if (v == ff95Layout) {
			this.initFF95Array();
			title = getString(R.string.Parameter_SetBroadcastCycle);
			type = FF95;
		} else if (v == ff97Layout) {
			this.initFF97Array();
			title = getString(R.string.Parameter_SettingTransmitPower);
			type = FF97;
		} else if (v == ff99Layout) {
			this.initFF99Array();
			title = getString(R.string.Remote_expansion_channel);
			type = FF99;
		} else if (v == ff9aLayout) {
			this.initFF9AArray();
			title = getString(R.string.System_function_switch);
			type = FF9A;
		}
	...
	private void initFF94Array() {
		...
		SetItem item = new SetItem();
		item.numberID = 0x55;
		item.name = getString(R.string.remote_resetControl);
		item.message = "……";
		array.add(item);
		SetItem item1 = new SetItem();
		item1.numberID = 0x35;
		item1.name = getString(R.string.Shallow_Recovery_For_RemoteControl);
		item1.message = "……";
		array.add(item1);
		SetItem item2 = new SetItem();
		item2.numberID = 0x36;
		item2.name = getString(R.string.Deep_Recovery_For_RemoteControl);
		item2.message = "……";
		array.add(item2);
	}
	...
	private void initFF99Array() {
		...
		item.numberID = 0x01;
		item.name = "0x01";
		item.message = getString(R.string.IO_trigger_control);
		array.add(item);
		SetItem item1 = new SetItem();
		item1.numberID = 0x02;
		item1.name = "0x02";
		item1.message = getString(R.string.Remote_shutdown_control);
		array.add(item1);
		SetItem item2 = new SetItem();
		item2.numberID = 0x03;
		item2.name = "0x03";
		item2.message = getString(R.string.Remote_bluetooth_request);
		array.add(item2);
		SetItem item3 = new SetItem();
		item3.numberID = 0x04;
		item3.name = "0x04";
		item3.message = getString(R.string.custom_radio_data_control);
		array.add(item3);
	}
