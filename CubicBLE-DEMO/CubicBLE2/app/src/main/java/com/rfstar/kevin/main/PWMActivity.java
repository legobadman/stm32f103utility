...
public class PWMActivity extends BaseActivity implements
	...
	private void initView() {
		...
		this.contentLayout.addView(this.outSingalseekBarInit(outSignalSeek,
				getString(R.string.signal_Frequency_Setting)));
		this.contentLayout.addView(this.changeTimeWidthseekBarInit(
				changeTiemSeek, getString(R.string.transition_Duration)));
	}
	...
	private View seekBarInit(SeekBar bar, String label) {
		...
		LinearLayout.LayoutParams numParams = new LayoutParams(-1, -2);
	...
	private View changeTimeWidthseekBarInit(SeekBar bar, String label) {
		...
		LinearLayout.LayoutParams btnParamsOff = new LayoutParams(-1, 81, 1);
		LinearLayout.LayoutParams btnParamsOn = new LayoutParams(-1, 81, 1);
		...
		offBtn.setText(getString(R.string.close));
		onBtn.setText(getString(R.string.open));
		onBtn.setTag(true);
		offBtn.setTag(false);
	...
	public void onClick(View v) {
		...
		if (v instanceof Button) {
			...
			byte[] data = new byte[4];
			if (boo) { // 开
				showMessage(getString(R.string.open));
				data[0] = (byte) PWM1Seek.getProgress();
				data[1] = (byte) PWM2Seek.getProgress();
				data[2] = (byte) PWM3Seek.getProgress();
				data[3] = (byte) PWM4Seek.getProgress();
			} else { // 关
				showMessage(getString(R.string.close));
				data[0] = (byte) 0xff;
				data[1] = (byte) 0xff;
				data[2] = (byte) 0xff;
				data[3] = (byte) 0xff;
